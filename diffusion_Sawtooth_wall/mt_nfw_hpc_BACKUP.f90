...
IMPLICIT NONE
!PUBLIC CONSTANTS
INTEGER,PARAMETER::X_MAX    =501            !DOMAIN SIZE IN X AXIS
INTEGER,PARAMETER::X_MIN    =1              !DOMAIN SIZE IN X AXIS
INTEGER,PARAMETER::Y_MAX    =51            !DOMAIN SIZE IN Y AXIS
INTEGER,PARAMETER::Y_MIN    =1              !DOMAIN SIZE IN Y AXIS
INTEGER,PARAMETER::Y_BOUND  =20             !FRACTURE BOTTOM Y POSITION
...
REAL(KIND=8),PARAMETER::LX  =10.0            !DOMAIN SIZE IN Y AXIS
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
OPEN(12,FILE='sawtooth_h0.1_mesh40.txt')
READ(12,*) TITLE
WRITE(*,*) TITLE
DO I=X_MIN+2,X_MAX-2
    READ(12,*) NUM(I),SURF(I)
ENDDO
SURF(X_MIN:X_MIN+1)=0
SURF(X_MAX-1:X_MAX)=0
!DO I=X_MIN,X_MAX-1
...
DO I    =   X_MIN,X_MAX
DO J    =   Y_MIN,Y_MAX-1
IF(IMAGE(I,J).EQ.0)THEN !NORTH(N),SOUTH(S),WEST(W),EAST(E)
...
USE LB_mass_transfer
IMPLICIT NONE
REAL(KIND=8) TEMP1,CU,U2,T1,T2,L_R
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
D_COEF  =   1.8D-3                  !! mass diffusion coeficient
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!      VELOCITY BOUND INITIAL     !!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
RE  	=   5.0D0                   !Reynolds NUMBER
...
OMEGA   =   1.0/TAU
U_TEM   =   U0_IN
ELSEIF(BOUND_FLAG.EQ.1)THEN
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!      PRESSURE BOUND INITIAL    !!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
TAU     =   0.8
NIU     =   (TAU-0.5)*DT*CS**2
OMEGA   =   1.0/TAU
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
CALL SEGMENTATION
Y_B_IN  =   FLOOR(SURF(X_MIN)/DY)
Y_B_OUT =   FLOOR(SURF(X_MAX)/DY)
...
IMPLICIT NONE
!WEST&EAST
U(X_MIN,Y_MIN:Y_MAX)  =   U0_IN*IMAGE(X_MIN,Y_MIN:Y_MAX)!U(X_MIN+1,Y_MIN:Y_MAX)!
V(X_MIN,Y_MIN:Y_MAX)  =   0.0	!V(X_MIN+1,Y_BOUND:Y_MAX)
RHO(X_MIN,Y_MIN:Y_MAX)=   DEN   !RHO(X_MIN+1,Y_MIN:Y_MAX)

U(X_MAX,Y_MIN:Y_MAX)  =   U(X_MAX-1,Y_MIN:Y_MAX)
V(X_MAX,Y_MIN:Y_MAX)  =   0.0   !V(X_MAX-1,Y_MIN:Y_MAX)
RHO(X_MAX,Y_MIN:Y_MAX)=   DEN
DO K=1,Q_S
DO J=Y_MIN,Y_MAX
...
!NORTH:DU/DY=0
DO I=X_MIN,X_MAX
    F(I,Y_MAX,4)=	F(I,Y_MAX-1,4)
    F(I,Y_MAX,7)=	F(I,Y_MAX-1,7)
    F(I,Y_MAX,8)=	F(I,Y_MAX-1,8)
ENDDO
DO K=1,Q_S  
!&SOUTH: NON-SLIP
DO I=X_MIN,X_MAX
...
ENDDO
ENDDO
!EAST: DC/DX=0  /or C=0.0
DO K=1,Q_S
DO J=Y_MIN,Y_MAX
IF(IMAGE(X_MAX,J).NE.0)THEN
    !G(X_MAX,J,K)    =   G(X_MAX-1,J,K)
    G(X_MAX,J,K)    =   -G(X_MAX,J,OPP(K))
ENDIF
ENDDO
ENDDO
!NORTH: DC/DX=0
...
WRITE(1,*)'VARIABLES=X,U,C,Den'
DO J=Y_MIN,Y_MAX
    WRITE(1,*) Y(X_MAX/2,J),U(X_MAX/2,J),C(X_MAX/2,J),RHO(X_MAX/2,J)
ENDDO
CLOSE(1)
